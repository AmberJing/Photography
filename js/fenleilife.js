$(function(){
	...
	// 加载更多
	$('#loading .loading-btn').click(function(){

		// 没有正在执行的定时器
		if(!flag){

			$('#loading .loading-btn .loading-img').css('display','block');
	...
	function loadMore(){
			...
			for(var i = 0; i < times; i ++){
				...
				var boxObj = '<div class="box">'
							+'  <a class="item_img">'
							+'	  <img src="'+ fluid[boxIndex].item_img +'"  width="285px" alt="">'
							+'	  <div class="dialog">'
							+'	     <h2>'+ fluid[boxIndex].dialog_txt +'</h2>'
							+'	     <span>'+ fluid[boxIndex].dialog_span +'</span>'
							+'	  </div>'
							+'	</a>'
							+'	<div class="item_author">'
							+'	  <img class="author_img" src="'+ fluid[boxIndex].head +'" width="50px" alt="">'
							+'	  <h4>'+ fluid[boxIndex].name +'</h4>'
							+'	  <span>'+ fluid[boxIndex].state +'</span>'
							+'	</div>'
							+'	<div class="item_state" id="item_state" style="display:none;">'
							+'	  <span>'+ fluid[boxIndex].author_state +'</span>'
							+'	</div>'
							+'</div>';
